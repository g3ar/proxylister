"""Restore a ProxyLister clone to its post-clone runtime state.

This command removes only artifacts owned by ProxyLister or common Python test
...
from proxylister.paths import lock_path, tool_home
...
def main() -> int:
    ...
    try:
        ...
        print(f"proxylister: refusing to clear: {error}", file=sys.stderr)
        return 1
