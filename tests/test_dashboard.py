...
class DashboardTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_repeated_click_and_browser_action_release_selection(self):
        ...
        async with app.run_test(size=(80, 12)) as pilot:
            ...
            await pilot.click("#table", offset=(70, 1))
            await pilot.pause()
            self.assertTrue(table.show_cursor)
            self.assertEqual(table.cursor_row, 0)

            await pilot.click("#table", offset=(70, 4))
            await pilot.pause()
            self.assertFalse(table.show_cursor)
    ...
    async def test_snapshot_populates_table_and_opens_details(self):
        ...
        async with app.run_test() as pilot:
            ...
            await pilot.click("#table", offset=(70, 1), times=2)
            await pilot.pause()
            self.assertIsInstance(app.screen, ProxyDetailsScreen)
            await pilot.click("#proxy-details", offset=(1, 1))
            await pilot.pause()
            self.assertIsInstance(app.screen, ProxyDetailsScreen)
            await pilot.click(offset=(0, 0))
            await pilot.pause()
            self.assertNotIsInstance(app.screen, ProxyDetailsScreen)
